...
public:
     ...
     * */
    hc__msg_parser_node(unsigned int hz, hc__device_connector *device_connector, std::shared_ptr<rclcpp::Node> nh, std::shared_ptr<rclcpp::Node> private_nh, string node_name, bool enable_read = true, bool enable_write = true)
    {
        if (NULL != nh && NULL != private_nh && NULL != device_connector)
        {
            ...
            this->enable_write = enable_write;
    ...
    void node_start(void)
    {
        ...
        if (!this->enable_read)
        {
            RCLCPP_INFO(nh->get_logger(), "write-only node started, waiting for write callbacks");
            while (rclcpp::ok())
            {
