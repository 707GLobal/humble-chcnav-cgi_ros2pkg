...
int main(int argc, char **argv)
{
    ...
    signal(SIGPIPE, SIG_IGN);     // 忽略 SIGPIPE，避免向已断开的对端 write 时进程被杀，导致 server 无法重连
    ...
    private_nh->declare_parameter<bool>("enable_read", enable_read);
    private_nh->declare_parameter<bool>("enable_write", enable_write);
    private_nh->get_parameter<bool>("enable_read", enable_read);
    private_nh->get_parameter<bool>("enable_write", enable_write);
    ...
    else if (type.compare("tcp") == 0)
    {
        ...
        RCLCPP_INFO(nh->get_logger(), "tcp client config host[%s] port[%d]", host.c_str(), port);

        device_connector = new tcp_common(host, port);
    }
    else if (type.compare("tcp_server") == 0)
    {
        string host = "";
        int port = 0;
        private_nh->declare_parameter<std::string>("host", host);
        ...
        RCLCPP_INFO(nh->get_logger(), "tcp server config host[%s] port[%d]", host.c_str(), port);

        device_connector = new tcp_common(host, port, true);
    }
   ...
   // 获取节点名
    ...
    // 初始化信息解析节点
    hc__msg_parser_node parser_node(rate, device_connector, nh, private_nh, node_name, enable_read, enable_write);
